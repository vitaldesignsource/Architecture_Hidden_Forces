import type { STATIONS } from "@/lib/ecology";

/**
 * The lexicon of the Hidden Ecology as data: every coined term, grouped by
 * root, with its definition and the page where it is at work. The lexicon
 * page sets it out; search reads it, so a term can be found from any page.
 */
export type Where = { to: (typeof STATIONS)[number]["to"] | "/ecology" | "/phos"; hash?: string; label: string };
export type Term = { k: string; root?: string; d: string; at?: Where };

export const FAMILIES: { id: string; k: string; root: string; note: string; backdrop?: string; portrait?: boolean; terms: Term[] }[] = [
  {
  ...
  },
  {
    ...
    terms: [
      ...
      { k: "The Menagerie of Impossible Forms", d: "Unprecedented symbolic organisms recombined from submerged Vestigia of unrelated lineages: what the Aquifer breeds beneath the Catacombs, and the Garden is where its recombinations first acquire allure. Named by the system, and reached through the Aquifer until it has a province of its own.", at: { to: "/ecology/aquifer", hash: "eco-a-catacombs", label: "The Aquifer" } },
    ],
  },
];

/** Every term, flat, with the family it belongs to. */
export const TERMS = FAMILIES.flatMap((f) => f.terms.map((t) => ({ ...t, family: f.k, familyId: f.id })));
