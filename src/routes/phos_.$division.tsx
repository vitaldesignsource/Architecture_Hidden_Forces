...
import { division, entriesOf, loadIntro, loadCoda, neighbourDivisions, progress, divisionLabel } from "@/lib/phos/entries";
...
export const Route = createFileRoute("/phos_/$division")({
  loader: async ({ params }) => {
    ...
    const [intro, coda] = await Promise.all([loadIntro(d.id), loadCoda(d.id)]);
    return { intro, coda };
  },
  head: ({ params }) => {
    const d = division(params.division);
...
function DivisionPage() {
  ...
  const { intro, coda } = Route.useLoaderData();
  useScrollTop(id);
  const d = division(id)!;
      ...
      <section className="relative isolate border-t border-border py-16">
        ...
        <div className="relative mx-auto max-w-6xl px-6">
          ...
          <div className="mt-16 grid gap-4 border-t border-border pt-8 sm:grid-cols-2">
            {prev && prev.id !== "portal" ? (
              <Link to="/phos/$division" params={{ division: prev.id }} className="group">
              ...
              <Link to="/phos/portal" className="group">
            ...
            {next && (
              <Link to="/phos/$division" params={{ division: next.id }} className="group sm:text-right">
