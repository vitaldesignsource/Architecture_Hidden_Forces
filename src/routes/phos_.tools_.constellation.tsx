      ...
      <ToolBand>
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="find a star by title…" aria-label="Find a star by title"
              className="w-56 border border-border bg-void px-3 py-1.5 font-mono text-[11px] text-bone placeholder:text-muted-foreground focus:border-gold focus:outline-none" />
            {(view.w !== W || view.x !== 0 || view.y !== 0) && (
              <button onClick={() => setView({ x: 0, y: 0, w: W })} className="border border-border px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground hover:border-gold/60">
                whole sky
              </button>
            )}
          </div>
        ...
        <div className="relative mt-8 overflow-hidden border border-border bg-void">
          <svg ref={svgRef} viewBox={`${view.x} ${view.y} ${view.w} ${view.w}`} className="aoh-sky mx-auto block h-auto w-full select-none"
            ...
            {/* division names at their constellations' centres */}
            {centroids.map((c) => {
            ...
            {hovNode && hovEntry && (
              <g pointerEvents="none">
                ...
                <text x={hovNode.x + 9 * fs + 3} y={hovNode.y + 9 * fs + 3} className="font-mono" fontSize={7.5 * fs + 1.2} letterSpacing={1 * fs} fill="var(--gold)" style={{ paintOrder: "stroke" }} stroke="var(--void)" strokeWidth={3 * fs + 1}>
          ...
          {DIVISIONS.map((dv) => {
            ...
            return (
              <button key={dv.id} onClick={() => setDiv(on ? null : dv.id)} aria-pressed={on} title={dv.title}
                className={`border px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.12em] transition-colors ${on ? "border-gold text-gold" : "border-border text-muted-foreground hover:border-gold/60"}`}>
                {dv.numeral || "Portal"}
              </button>
            );
          })}
          {div && <button onClick={() => setDiv(null)} className="px-2.5 py-1 font-mono text-[10px] uppercase tracking-[0.12em] text-gold-dim hover:text-gold">clear</button>}
        </div>
      </ToolBand>
          ...
          {mostCited.map((n, i) => {
            ...
            return (
              <Link key={n.id} to="/phos/$division/$entry" params={{ division: e.division.id, entry: e.slug }}
                ...
                <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-gold-dim">{String(i + 1).padStart(2, "0")}</span>
                <span className="min-w-0">
                  <span className="block font-serif text-lg text-bone transition-colors group-hover:text-gold">{e.title}</span>
                  <span className="mt-0.5 block font-mono text-[9px] uppercase tracking-[0.2em] text-gold-dim">{e.division.numeral || "Portal"} · {String(e.n).padStart(2, "0")}</span>
                </span>
                <span className="font-mono text-[10px] uppercase tracking-[0.15em] text-bone/60">cited by {n.in}</span>
              </Link>
            );
          })}
        </div>
