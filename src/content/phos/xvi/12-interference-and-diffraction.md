---
title: Interference and Diffraction
labels: [Scientific Evidence, Architecture of Hidden Forces Doctrine]
confidence: [Established]
tradition: [Scientific, Architecture of Hidden Forces]
period: [Modern]
plane: [Physical, Psychic]
operation: [Differentiation, Withdrawal]
symbol: [Circle]
related: [xvi-7, xvi-13, vi-11, vi-2, x-12, xvi-10]
summary: Amplitudes reinforcing or cancelling according to phase, and waves spreading as they pass apertures or obstacles - a fundamental limit on focusing, and darkness produced by the ordered interaction of light with light rather than by absence.
epigraph: "Not every dark region signifies absence, evil, or weak energy."
backdrop: scrims
---

Interference occurs when wave amplitudes overlap. In constructive interference, compatible phases reinforce and produce greater intensity. In destructive interference, opposed phases reduce or cancel the resulting amplitude. The energy has not simply vanished; it is redistributed through the total interference field.

Diffraction is the spreading and restructuring of waves as they pass through apertures or around obstacles. It becomes especially important when the aperture or obstacle is comparable in size to the wavelength. A narrow opening does not transmit a perfectly narrow geometrical ray; it produces a spreading pattern.

This places a fundamental limit on focusing and image resolution. An aperture that seems capable of isolating a finer ray eventually produces greater diffraction. Perfect localisation cannot be achieved simply by narrowing the opening indefinitely.

## Darkness made by light

Interference and diffraction reveal that obstacles do more than block light. They help determine the structure of its manifestation. Edges, separations, and apertures generate patterns. Darkness within an interference image may be produced by the ordered interaction of light with light.

This is an especially valuable correction to esoteric symbolism. Not every dark region signifies absence, evil, or weak energy. Darkness can arise through phase opposition within an active field. Two powerful currents may produce local cancellation because their relations are incompatible.

Psychologically, this resembles internal conflict. Strong forces can be present while their visible result is paralysis. The solution may not be to add more force, but to restore phase, timing, and right relation.

:: In the Architecture: [§ XLIV · The Law of Right Relation](/#relation) and [§ XII · The Laws of Formation](/#laws).
